...
if isempty(dir('Floes_bnds5')); disp('Creating folder: Floes_bnds5'); mkdir('Floes_bnds5'); end
if isempty(dir('./Floes_bnds5/figs')); disp('Creating folder: figs'); mkdir('./Floes_bnds5/figs'); end
...
%            [fig] =plot_basic(fig, Time,Floe,ocean,c2_boundary_poly,Nb,PERIODIC);
            exportgraphics(fig,['./Floes_bnds5/figs/' num2str(im_num,'%03.f') '.jpg']);
        end
        ...
        if ifPlotStress
            ...
            saveas(fig,['./Floes_bnds5/figs/' num2str(im_num,'%03.f') '.jpg'],'jpg');
%             figure(2)
%             plot(Xc,SigXXa,'kx','linewidth',2); title(['Time = ' num2str(Time/3600) ' hours'],'fontsize',24);
    ...
    if mod(i_step,nDTOut)==0
        save(['./Floes_bnds5/Floe' num2str(im_num,'%07.f') '.mat'],'Floe','Nb','Nbond','eularian_data','SigXXa','SigXYa', 'SigYYa','U','V','dU','Fx','mass','c2_boundary');
        SigXX = zeros(Ny, Nx); SigYX = zeros(Ny, Nx);
        SigXY = zeros(Ny, Nx); SigYY = zeros(Ny, Nx);
        DivSigX = zeros(Ny, Nx); DivSig1 = zeros(Ny, Nx);
        DivSigY = zeros(Ny, Nx); DivSig2 = zeros(Ny, Nx);
