function output = looped_figs(kk)
kk = kk;
fig = 0; num_chunks = zeros(1,427);Nbound = 0;edges = 0:2:102;
load(['/dat1/egonzalez/floes_OG_shape_2/Floe' num2str(kk,'%07.f') '.mat'])
...
%box on
fig = figure(1);
exportgraphics(fig,['./figs_shape_2/' num2str(kk,'%03.f') '.jpg'] ,'resolution',300);
...
x2 = bins(19);
y2 = FSD(19);
m = (log10(y2) - log10(y1)) / (log10(x2) - log10(x1));

fig(5) = loglog([bins(1,10),bins(1,19)],[FSD(1,10),FSD(1,19)],'r--','linewidth',2);
legend('FSD','Slope = -2',sprintf('Slope = %.2f', m));

fig2 = figure(3);
%exportgraphics(fig2,['./plotting/FSD.jpg'] ,'resolution',300);
exportgraphics(fig2,['./figs_shape_2/FSD' num2str(kk,'%03.f') '.jpg'],'resolution',300);
%save(['./FSD' num2str(kk) '.mat'], 'FSD');
