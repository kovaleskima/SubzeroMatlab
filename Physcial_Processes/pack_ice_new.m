function [Floe,Vd] = pack_ice_new(Floe,c2_boundary,dhdt,Vd,target,ocean, height, min_floe_size, PERIODIC,Nx,Ny,Nb)
...
% poolobj = gcp;
% addAttachedFiles(poolobj,{'ArcticPolyshapes.mat'})
                ...
                if subfloes(iii).NumHoles > 0
                    ...
                    if sum(area(intersect(poly2,[Floe(live==1).poly])))>0
                        ...
                        for kk = 1:length(in)
                            ...
                            tmpFloe = FuseFloes(floe2(j).floe(OverlappingFloe),Floe(in(kk)));
                            if length(tmpFloe) > 1
